...
router.get("/show/:router", (req, res) => {
  ...
  con.query("SELECT * FROM result WHERE router =?", router, (err, row) => {
    if (err) throw err;
    var dict = {};
...
router.post("/showform", (req, res) => {
  ...
  con.query(
    "SELECT title,router FROM form WHERE userid = ?",
    userid,
    (err, rows) => {
      if (err) throw err;
      res.send({ data: rows });
...
router.post("/deleteform", (req, res) => {
  ...
  con.query("DELETE FROM form WHERE router = ?", router, (err, rows) => {
    if (err) throw err;
    res.send({ data: rows });
  });
});
